...
class Player(metaclass=abc.ABCMeta):
    ...
    def make_move(self, game_state: GameState) -> GameState:
        if self.mark is game_state.current_mark:
            ...
            raise InvalidMove('It\'s other player\'s turn')
...
class CompyterPlayer(Player, metaclass=abc.ABCMeta):
    def __init__(self, mark: Mark, delay_seconds: float = .25) -> None:
        super().__init__(mark)
...
class RandomComputerPlayer(CompyterPlayer):
    def get_computer_move(self, game_state: GameState) -> Move | None:
        try:
